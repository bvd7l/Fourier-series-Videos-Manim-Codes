...
def main_eq(A, pos=None):
    eq = MathTex(f"f(t) = {A.get_value():.1f} \\sin(t)", font_size=40)
    # if pos is not None:
...
class SinAm(Scene):
    def construct(self):
        ...
        fx_sin_tex = MathTex("f(t)=Asin(\omega t)", font_size=40)
        fx_sin_tex.to_edge(UP, buff=1)
        self.play(Write(fx_sin_tex))
        self.wait()
        omega = MathTex("f(t)=Asin(t)", font_size=40).to_edge(UP, buff=1)
        ...
        # self.wait()
        equation3 = always_redraw(lambda: main_eq(Am, 1))

        self.play(
            # axes_graph.animate.shift(0.5 * UP),
            ReplacementTransform(omega, equation3),
        )
        self.wait(2)

        sin_graph = always_redraw(
            lambda: axes.plot(
                lambda x: fx_sin(Am.get_value(), 1, x),
                x_range=[0, 4 * PI],
                color=YELLOW,
                stroke_width=3,
            )
        ...
        amplitude_values = [0.5, 1.5, 2.0, 2.5, 1.8]

        for amp in amplitude_values:
            self.play(Am.animate.set_value(amp), run_time=2, rate_func=smooth)
            # self.wait()
        self.wait(2)
        ...
        definition = ar_text("المطال: هو ارتفاع التابع عن المحور المركزي.", 30).next_to(
            axes, DOWN, buff=0.5
        )
