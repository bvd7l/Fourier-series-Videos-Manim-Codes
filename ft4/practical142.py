...
class Part42(Scene):
    ...
    def construct(self):
        ...
        dashed = DashedLine(
            start=big_axes.c2p(0, 2),
            end=big_axes.c2p(10 * PI, 2),
            color=WHITE,
            stroke_opacity=0.5,
            dash_length=0.09,
            dashed_ratio=0.5,
        ...
        small_axes_list = [self.Create_small_axes_copy() for _ in range(3)]
        ...
        amplitude_value = 2
        points = [
            (i * PI, 0 if i & 1 else amplitude_value)
        ...
        for i in range(2):
            fr.append((2 * i + 1))
            Amm.append(8 / mkam(2 * i + 1))

        col = [PINK, GREEN, ORANGE]
        ...
        for i, (ax_g, f, a_m, cl) in enumerate(zip(all_small_axes, fr, Amm, col)):
            ax = ax_g[0]
            if i == 0:
                wave_func = lambda x: f_one(x)
            else:
                wave_func = lambda x, freq=f, AA_m=a_m: f_cos(x, f, AA_m) + 1.1
            sin_wave = ax.plot(
                wave_func,
                x_range=[0, 10 * PI],
                color=cl,
        ...
        sin_wave_2 = (
            MathTex(r"\frac{8}{ \pi^2} \cos(t)", font_size=30)
            .next_to(small_axes_list[1], RIGHT, buff=1)
            .shift(0.03 * UP)
        )
        sin_wave_3 = (
            MathTex(r"\frac{8}{9 \pi^2} \cos(3\pi t)", font_size=30)
        ...
        #     .shift(0.03 * UP)
        # )
        sin_wave_formulas.add(sin_wave_1, sin_wave_2, sin_wave_3)
        for sw in range(3):
            sin_wave_formulas[sw].set_color(col[sw])
        ...
        self.wait()
        num = 4
        fr_f = [(2 * i + 1) for i in range(num)]
        Amm_f = [8 / mkam(2 * i + 1) for i in range(num)]

        def the_final_signal(x):
            tot = 0
            for i in range(num):

                tot += f_cos(x, fr_f[i], Amm_f[i])
            return tot + f_one(x)

        graphing_f = main_axes.plot(
            the_final_signal, x_range=[0, 10 * PI], color=BLUE, stroke_width=3.5
        )
